...
3. the planned intent matches an independent deterministic classification of the
   user question;
...
plan is recorded as `semantic_mismatch`, corrected by the deterministic guard,
and fails the LLM plan-quality release gate.
...
`filing-investigation-evaluator-v2`, records its report in
...
9. Ask **For which stocks do you have data?** and show the full represented and
