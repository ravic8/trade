...
def run_baseline_predictions_v1_pipeline(
    ...
    output_dir = settings.data_dir / "processed/ml/baselines_v1"
    ...
    predictions_path = predictions_output or output_dir / "baseline_predictions.parquet"
    metrics_path = metrics_output or output_dir / "baseline_metrics.json"
    summary_path = summary_output or output_dir / "baseline_summary.md"

    dataset, feature_columns = load_ml_dataset_v1(
        dataset_path=dataset_file,
        feature_columns_path=feature_columns_file,
    )
    result = run_baseline_predictions(dataset, feature_columns, config=config)
    ...
    warnings = [] if status == "pass" else ["No baseline prediction rows generated."]
    return PipelineRunResult(
        name="baseline_predictions_v1",
        status=status,
