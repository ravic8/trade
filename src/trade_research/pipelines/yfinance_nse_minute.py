...
def run_yfinance_nse_minute_pipeline(
    *,
    ...
    start = _parse_datetime(from_datetime, "from_datetime") if from_datetime else request_floor
    if start >= end:
    ...
    requested_symbols = _normalize_provider_symbols(provider_symbols)
    if requested_symbols and symbol_limit is not None:
        raise ValueError("provider_symbols and symbol_limit cannot be used together.")
    ...
    selected_rows = _select_instrument_rows(
        instrument_rows,
    ...
    run_id = db.start_ingestion_run(
        ...
        run_metadata={
            ...
            "selected_provider_symbols": [instrument.yahoo_symbol for instrument in instruments],
        },
    )
    ...
    try:
        ...
        observed_availability_sessions, unavailable_reason_codes = availability_session_maps(
            availability_observations
        )
        observed_session_windows = nse_minute_observed_session_windows(
            raw_frame,
            eligible_sessions,
        )
        ...
        status = (
            ...
            if failures
            else ("completed" if clickhouse_rows else "completed_empty")
        )
        ...
        db.finish_ingestion_run(
            ...
            items_processed=0,
            items_succeeded=0,
            items_failed=len(instruments),
            error_message=str(exc),
        )
        raise
    warnings = [
        *([f"yfinance NSE minute fetch recorded {len(failures)} failures."] if failures else []),
...
def _normalize_provider_symbols(provider_symbols: Iterable[str] | None) -> list[str]:
    normalized: list[str] = []
    seen: set[str] = set()
    for value in provider_symbols or ():
        ...
        if "." not in symbol:
            symbol = f"{symbol}.NS"
        if symbol in seen:
            raise ValueError(f"Duplicate NSE provider symbol requested: {symbol}.")
        seen.add(symbol)
        normalized.append(symbol)
    return normalized


def _select_instrument_rows(
    ...
    *,
    ...
    if not requested_symbols:
        return instrument_rows[:selected_limit]
    if len(requested_symbols) > configured_limit:
        raise ValueError(
            "Requested NSE provider symbol count exceeds the configured per-run limit: "
            f"{len(requested_symbols)}>{configured_limit}."
        )
    rows_by_symbol = {str(row.get("provider_symbol") or "").upper(): row for row in instrument_rows}
    missing = [symbol for symbol in requested_symbols if symbol not in rows_by_symbol]
    if missing:
        raise ValueError("No active NSE yfinance instrument matched: " + ", ".join(missing))
    return [rows_by_symbol[symbol] for symbol in requested_symbols]
